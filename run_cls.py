...
from data_loader import CLSProcessor, IEProcessor, NSProcessor, PJProcessor, TFProcessor
...
def evaluate(args, model, tokenizer, prefix="", output_pred=False):
    ...
    if output_pred:
        ...
        with open(output_pred_file, "w") as writer:
            with open(os.path.join(args.data_dir, args.dev_path)) as f:
                lines = f.read().split('\n')
                result = []
                for pred, line in zip(preds, lines):
                    result.append(str(pred) + '\t' + line)
                writer.write('\n'.join(result))
        return preds
    else:
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        get_examples = {'pred': processor.get_pred_examples,
                        'dev': processor.get_dev_examples,
                        'train': processor.get_train_examples}[set_type]

        examples = get_examples(os.path.join(args.data_dir, args.dev_path) if evaluate
                                else os.path.join(args.data_dir, args.train_path))
...
def main():
    ...
    ## Other parameters
    parser.add_argument("--dev_path", default="dev.txt", type=str)
    parser.add_argument("--train_path", default="train.txt", type=str)
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        model = model_class.from_pretrained(args.output_dir)
        model.to(args.device)
        preds = evaluate(args, model, tokenizer, output_pred=True)
